...
class Media {
  ...
  onResize({
    ...
    this.plane.scale.y =
      (this.viewport.height * (1400 * this.scale)) / this.screen.height;
    this.plane.scale.x =
      (this.viewport.width * (1100 * this.scale)) / this.screen.width;
...
class App {
  ...
  handleClick() {
    ...
    // Find the center media item (currently in focus)
    if (this.medias && this.medias.length > 0) {
      const centerIndex = Math.round(Math.abs(this.scroll.current) / this.medias[0].width) % this.mediasImages.length;
      const centerMedia = this.mediasImages[centerIndex];
      if (centerMedia) {
        this.onItemClick(centerMedia);
      }
    }
  }
